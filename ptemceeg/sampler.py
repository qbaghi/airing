...
class LikePriorEvaluator(object):
    ...
    def __call__(self, x, x2):
        ...
        if lp == float('-inf'):
            ...
            ll = self.logl(x, x2, *self.loglargs, **self.loglkwargs)
            if np.isnan(ll).any():
...
class GibbsEvaluator(object):
    ...
    def __call__(self, x, x2):
        ...
        ----------
        x : ndarray
            primary sampled parameters
        x2 : ndarray
            auxiliary Gibbs parameters
...
class Sampler(object):
    ...
    def run(self, n_it, n_save, n_thin, n_update,
        ...
        aux0 : array_like
            initial values for auxiliary parameters nwalkers x ndim2
        save_path : str
    ...
    def sample(self, p0=None, aux0=None,
        ...
        * ``ratios``, the instantaneous inter-chain acceptance ratios
        ...
        if aux0 is not None:
            ...
            raise ValueError('Initial walker auxiliary param not specified.')
        ...
        if self._logposterior0 is None or self._loglikelihood0 is None:
            logl, logp = self._evaluate(p, aux)
