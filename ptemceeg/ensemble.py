...
class Ensemble(object):
    """
    This contains as little contextual information as it can.  
    It represents an ensemble.py that performs steps in the
    parameter space.

    """

    _config = attr.ib(type=EnsembleConfiguration, 
    ...
    # Initial walker auxiliary parameters
    x2 = attr.ib(type=np.ndarray, converter=np.array)
    ...
    _random = attr.ib(type=RandomState, validator=instance_of(RandomState), 
                      factory=RandomState)
    ...
    ndim2 = attr.ib(type=int, init=False)
    ...
    def __attrs_post_init__(self):
        ...
        if self.logP is None or self.logl is None:
            logl, logp = self._evaluate(self.x, self.x2)
    ...
    def step(self):
        self._stretch(self.x, self.x2, self.logP, self.logl)
    ...
    def _stretch(self, x, x2, logP, logl):
        """
        Perform the stretch-move proposal on each ensemble.py.
        Ensure that association between primary x and auxiliary x2 
        parameters is kept. 
        ...
        for j in [0, 1]:
            ...
            y_logl, y_logp = self._evaluate(y, x2)
    ...
    def update_aux(self, x, x2):
        """
        Perform a Gibbs step updating the auxiliary parameters x2 conditionnaly
        to the primary parameters x.
        
        Should return the updated x2.

        """

        # Make a flattened iterable of the results, of alternating logL and logp.
        values = x.reshape((-1, self.ndim))
        # Same for auxiliary parameters
        shape2 = x2.shape[:-1]
        values2 = x2.reshape((-1, self.ndim2))
        length2 = len(values2)
        results = itertools.chain.from_iterable(
            self._mapper(self._config.gibbs, values, values2))

        # Construct into a pre-allocated ndarray.
        array = np.fromiter(results, float, 2 * length2).reshape(shape2 + (2,))
        return tuple(np.rollaxis(array, -1))

    def _evaluate(self, x, x2):
        """
        Evaluate the log likelihood and log prior functions at the specified 
        walker positions, and with specified auxiliary parameters.
        ...
        values2 = x2.reshape((-1, self.ndim2))
        results = itertools.chain.from_iterable(
            self._mapper(self._config.evaluator, values, values2))
    ...
    def _temperature_swaps(self, x, logP, logl):
        """
        Perform parallel-tempering temperature swaps on the state in ``x`` 
        with associated ``logP`` and ``logl``.
